...
export const mobileProxyPagePlatform = {
  fixedTunMode: true,
  supportsSniffOverrideDestination: true,
  modeSwitchLabel: "VPN 模式",
  modeSwitchHelpContent: {
    effect:
      "Android 当前固定使用虚拟网卡（VPN/TUN）模式运行。点击“启动”后会先建立 VPN，再接管系统流量与 DNS；停止后系统网络与 DNS 将完全回归 Android 本身。",
    caution: "未启动代理时不会单独运行最小实例，因此不支持节点测速或真连评分。",
    recommendation: "如需执行节点评分或优选切换，请先启动代理，再在运行中的代理环境里操作。",
